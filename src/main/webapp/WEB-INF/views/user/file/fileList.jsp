			...
			<main class="page-content d-flex flex-column flex-row-fluid">
				...
				<div class="content py-4 px-4 px-lg-8 d-flex flex-column-fluid">
					...
					<div class="container-fluid px-0">
						...
						<!--Email Header-->
						<div class="top-0 mb-2">
							<div class="d-flex align-items-center pb-2 flex-row justify-content-between">

								<div class="flex-grow-1">

									<div data-bs-placement="top" data-bs-toggle="tooltip" title="삭제" class="border text-body hover-bg-secondary btn btn-sm shadow-sm" onclick="showDeleteModal();">
										<i class="bi bi-trash-fill"></i>
									</div>
									<a href="javascript:goFileForm('<c:out value="${item.hydcSeq}"/>','<c:out value="${vo.thisPage}"/>',
								'<c:out value="${vo.shHydcOption}"/>','<c:out value="${vo.shHydcValue}"/>');" data-bs-placement="top" data-bs-toggle="tooltip" title="등록" class="border text-body hover-bg-secondary btn btn-sm shadow-sm">
										<i class="bi bi-pencil-fill"></i>
									</a>
									<a href="javascript:goFileListTemp('<c:out value="${item.hydcSeq}"/>','<c:out value="${vo.thisPage}"/>',
								'<c:out value="${vo.shHydcOption}"/>','<c:out value="${vo.shHydcValue}"/>');" data-bs-placement="top" data-bs-toggle="tooltip" title="임시저장리스트" class="border text-body hover-bg-secondary btn btn-sm shadow-sm">
							...
							<div class="sticky-top top-0">
								...
								<div class="pb-5">
									<form class="position-relative">
										<input class="form-control form-control" type="text" name="shHydcValue" id="shHydcValue" style="" value="" placeholder=" 검색어를입력해주세요(제목,내욕,파일명,작성자)  UI공사중이에요~~">
										<button class="btn btn-primary" type="submit" name="search" id="btnSearch" style="display: none;"></button>
									</form>
								</div>
							</div>
						</div>
							...
							<div class="col-12">
								<div class="card">
									<div class="table-responsive card-body">
										<table class="table-card align-middle table-nowrap mb-0 table table-borderless">
											<tbody>
												<c:choose>
													...
													<c:otherwise>
														<c:forEach items="${list}" var="item" varStatus="status">
															<tbody id="">
																<tr>
																	<td>
																		<input type="checkbox" id="checkboxSeq" name="checkboxSeq" value="<c:out value="${item.hydcSeq}" />">
																	</td>

																	<td class="">
																		<c:choose>
																			<c:when test="${empty item.uuidFileName}">
																				<img style="width: 50px; height: 50px;" src="/resources/user/images/profileDefault.png" class="rounded-circle" alt="">
																			</c:when>
																			<c:otherwise>
																				<img style="width: 50px; height: 50px;" src="<c:out value="${item.path}"/><c:out value="${item.uuidFileName}"/>" class="avatar rounded-pill flex-shrink-0" />
																			</c:otherwise>
																		</c:choose>
																	</td>
																	...
																	<td class="">
																		<fmt:formatDate pattern="yyyy-MM-dd hh:mm" value="${item.regDateTime }" />
																	</td>
